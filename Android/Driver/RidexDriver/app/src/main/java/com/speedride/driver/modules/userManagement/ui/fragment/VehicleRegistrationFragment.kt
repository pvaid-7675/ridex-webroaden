...
class VehicleRegistrationFragment : BaseFragment(), ImagePickerListener, View.OnClickListener {
    ...
    private var mApiSendVehicleRegistrationCall: Call<ServerResponse<Data>>? = null
    ...
    override fun initView(view: View?) {
        ...
        mTxtMotorInsCertDetails!!.setText(resources.getString(R.string.certificate_registration_long_txt))
        mTxtMotorInsCertificate!!.setText(resources.getString(R.string.certificate_registration))
    ...
    override fun onViewCreate(view: View?, savedInstanceState: Bundle?) {
        ...
        mDocumentUploadActivity!!.setTitle(resources.getString(R.string.vehicle_registration))
    }
    ...
    private fun apiSendVehicleRegistration(imageFile: File?) {
        if (ValidationUtils.isValidString(mStrDriverId) && imageFile != null) {
            ...
            val docTypeBody : RequestBody = Common.DOC_REGISTRATION.toRequestBody("text/plain".toMediaType())
            ...
            mApiSendVehicleRegistrationCall = Singleton.restClient
                .uploadDriverVehicleDocumentsAll(idBody, docTypeBody, doc_image)
            mApiSendVehicleRegistrationCall!!.enqueue(object : Callback<ServerResponse<Data>> {
                    ...
                    if (resp != null && resp.status == 200) {
                        ...
                        if (ValidationUtils.isValidString(response.body()!!.data?.d_licence)) //capture image save in preference
                            mPreferenceUtils!!.save(
                                PreferenceUtils.PREF_KEY_DOCUMENT_VEHICLE_REGISTRATION,
                                response.body()!!
                                    .data?.v_registration
                            )
                    } else {
    ...
    override fun onClick(v: View) {
        when (v.id) {
            R.id.rlTakePhoto -> callImagePicker_new()
        }
    }
    ...
    private val launcher =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            if (it.resultCode == Activity.RESULT_OK) {
                ...
                apiSendVehicleRegistration(mImageFile)
            }
        }
    ...
    override fun getImageFileFromPicker(image: File?) {
        if (ValidationUtils.isInternetAvailable(activityContext!!)) {
            if (ValidationUtils.isValidObject(image)) {
                ...
                apiSendVehicleRegistration(image)
            }
        } else {
    ...
    private fun onCancelApiCall() {
        if (mApiSendVehicleRegistrationCall != null && !mApiSendVehicleRegistrationCall!!.isCanceled) {
            mApiSendVehicleRegistrationCall!!.cancel()
        }
    }

    companion object {
        private val TAG = VehicleRegistrationFragment::class.java.simpleName
    }
}