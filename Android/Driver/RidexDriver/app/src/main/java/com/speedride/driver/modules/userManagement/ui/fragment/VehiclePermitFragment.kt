...
class VehiclePermitFragment : BaseFragment(), ImagePickerListener, View.OnClickListener {
    ...
    private var mApiSendVehiclePermitCall: Call<ServerResponse<Data>>? = null
    ...
    override fun initView(view: View?) {
        ...
        mTxtMotorInsCertDetails!!.setText(resources.getString(R.string.contract_carriage_long_txt))
        mTxtMotorInsCertificate!!.setText(resources.getString(R.string.contract_carriage_permit))
    ...
    override fun onViewCreate(view: View?, savedInstanceState: Bundle?) {
        ...
        mDocumentUploadActivity!!.setTitle(resources.getString(R.string.vehicle_permit))
    }
    ...
    private fun apiSendVehiclePermit(imageFile: File?) {
        if (ValidationUtils.isValidString(mStrDriverId) && imageFile != null) {
            ...
            val docTypeBody : RequestBody = Common.DOC_PERMIT.toRequestBody("text/plain".toMediaType())
            ...
            mApiSendVehiclePermitCall = Singleton.restClient
                .uploadDriverVehicleDocumentsAll(idBody, docTypeBody, doc_image)
            mApiSendVehiclePermitCall!!.enqueue(object : Callback<ServerResponse<Data>> {
                    ...
                    if (resp != null && resp.status == 200) {
                        ...
                        if (ValidationUtils.isValidString(response.body()!!.data?.d_licence)) //capture image save in preference
                            mPreferenceUtils!!.save(
                                PreferenceUtils.PREF_KEY_DOCUMENT_VEHICLE_PERMIT,
                                response.body()!!
                                    .data?.v_permit
                            )
    ...
    private val launcher =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            if (it.resultCode == Activity.RESULT_OK) {
                ...
                apiSendVehiclePermit(mImageFile)
            }
        }
    ...
    private fun onCancelApiCall() {
        if (mApiSendVehiclePermitCall != null && !mApiSendVehiclePermitCall!!.isCanceled) {
            mApiSendVehiclePermitCall!!.cancel()
        }
    }

    companion object {
        private val TAG = VehiclePermitFragment::class.java.simpleName
    }
}