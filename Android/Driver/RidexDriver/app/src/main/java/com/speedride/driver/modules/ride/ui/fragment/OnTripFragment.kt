 ...
 */
class OnTripFragment : BaseFragment(), View.OnClickListener, OnMapReadyCallback, GpsListener,
    ...
    override fun onInfoWindowClick(marker: Marker) {
        if (ValidationUtils.isValidObject(markerDropOff) && marker == markerDropOff) {
            if (ValidationUtils.isValidObject(mPreferenceUtils!!.customerRideData) && ValidationUtils.isValidString(
                    mPreferenceUtils!!.customerRideData?.mobile
                )
            ) {
                val intent = Intent(Intent.ACTION_DIAL)
                intent.data = Uri.parse("tel:" + (mPreferenceUtils!!.customerRideData?.mobile))
    ...
    private inner class DownloadTask : AsyncTask<String?, Void?, String>() {
        ...
        override fun doInBackground(vararg url: String?): String {
            ...
            try {
                ...
                AppLog.d("Background Task", e.toString())
            }
            return data
        }
    }
    ...
    private fun currentLocation() {
        if (ActivityCompat.checkSelfPermission(
            ...
            if (mCurrentLocation != null && customerRequestModel != null) {
                ...
                if (mPickupLatLong != null && mDropOffLatLong != null) if (markerCount == 1 && markerPickup != null) {
                ...
                mGoogleMap.addMarker(getPickupLocationMarker(mPickupLatLong)).showInfoWindow();*/
                    ...
                    val strdistance = "" + DecimalFormat("##.##").format(distance) + " miles"
                    mTxtDistance!!.text = strdistance
    ...
    private fun apiConfirmRidePassCode() {
        ...
        Log.d(
            TAG,
            "apiConfirmRidePassCode: " + strPassCode + " bookid:" + (mPreferenceUtils!!.customerRideData?.bookid)
        )
        if (ValidationUtils.isValidString(strPassCode) && ValidationUtils.isValidString(
            ...
            mApiReviewRatingTripResponseCall = Singleton.restClient.confirmPassCodeRideTrip(
                mPreferenceUtils!!.customerRideData?.bookid, strPassCode
            )
    ...
    private fun apiStartRideWithCustomer() {
        ...
        Log.d(
            TAG,
            "apiStartRideWithCustomer bookid:" + (mPreferenceUtils!!.customerRideData?.bookid) + " cus.id:" + (mPreferenceUtils!!.customerRideData?.customer_id
                ) + " dri.id:" + (mPreferenceUtils!!.customerRideData?.driver_id)
        )
        if (ValidationUtils.isValidString(mPreferenceUtils!!.customerRideData?.bookid) && ValidationUtils.isValidString(
                mPreferenceUtils!!.customerRideData?.customer_id
            ) && ValidationUtils.isValidString(
               ""+ mPreferenceUtils!!.customerRideData?.driver_id
            )
        ) {
    ...
    private fun apiCompleteRideWithCustomer() {
        ...
        Log.d(
            TAG,
            "apiCompleteRideWithCustomer bookid:" + (mPreferenceUtils!!.customerRideData?.bookid) + " cus.id:" + (mPreferenceUtils!!.customerRideData?.customer_id) + " dri.id:" + (mPreferenceUtils!!.customerRideData?.driver_id)
        )
        if (ValidationUtils.isValidString(mPreferenceUtils!!.customerRideData?.bookid) && ValidationUtils.isValidString(
                mPreferenceUtils!!.customerRideData?.customer_id
            ) && ValidationUtils.isValidString(
...
*/
    ...
    fun ApiCallSharedData() {
        mApiAcceptRideList = Singleton.restClient.getSharedRide(mPreferenceUtils!!.customerRideData?.driver_id)
